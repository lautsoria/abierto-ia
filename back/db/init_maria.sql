...
SELECT 
  UUID(),
  ranked_reservations.usuario_id,
  ranked_reservations.servicio_id,
  ranked_reservations.id,
  FLOOR(3 + (RAND() * 3)),
  'Excelente trabajo, muy recomendable.',
  DATE_FORMAT(DATE_ADD(ranked_reservations.fecha_servicio, INTERVAL 1 DAY), '%Y-%m-%dT%H:%i')
FROM (
  SELECT 
    r.*,
    ROW_NUMBER() OVER (PARTITION BY r.servicio_id ORDER BY RAND()) as rn
  FROM reservas r
  WHERE r.estado = 'realizado'
) ranked_reservations
WHERE ranked_reservations.rn <= 3;
